...
export const contentToFormData = (
  ...
  if (attachments) {
    const localFiles = attachments.filter(isLocalFile).map((x) => x.file);
  ...
  if (image && isLocalImage(image)) {
    formData.append(
      type === 'CREATE' ? 'mainImage' : 'newMainImage',
      image.file,
    );
  }
...
export const getAttachmentDeleteIds = (
  ...
  const isNumberArr = (
    arr: number[] | Attachment[] | EditorFile[],
  ): arr is number[] => arr.length === 0 || typeof arr[0] === 'number';
  const isNumberAttachmentArr = (
    arr: Attachment[] | EditorFile[],
  ): arr is Attachment[] => arr.length === 0 || 'id' in arr[0];
