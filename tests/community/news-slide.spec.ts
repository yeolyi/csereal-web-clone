...
test('새소식 메인-슬라이드쇼 설정 테스트', async ({ page }, testInfo) => {
  const dateTimeString = getKoreanDateTime();
  const title = `슬라이드 새소식 ${dateTimeString}`;
  const description = `슬라이드 내용 ${dateTimeString}`;
  const date = new Date();
  ...
  await toggleCheckbox(page, '메인-슬라이드쇼에 표시');
