...
test('외국인 대학원 입학 편집 및 한/영 내용 검증', async ({ page }) => {
  const dateTimeString = getKoreanDateTime();
  const koText = `Playwright KO ${dateTimeString}`;
  const enText = `Playwright EN ${dateTimeString}`;

  // 1. 로그인
  await page.goto('/admissions/international/graduate');
  await loginAsStaff(page);

  // 2. 편집 페이지로 이동
  await page.getByRole('link', { name: '편집' }).click();
  await page.waitForURL('**/admissions/international/graduate/edit');
  ...
  await page.waitForURL('**/admissions/international/graduate');

  // 5. 검증 - 한글 페이지
  await expect(page.locator('body')).toContainText(koText);

  // 6. 검증 - 영문 페이지
  await switchPageLanguage(page, 'en', '/admissions/international/graduate');
