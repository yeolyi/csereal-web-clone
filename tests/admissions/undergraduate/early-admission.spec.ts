...
test('학부 수시 모집 편집 및 한/영 내용 검증', async ({ page }) => {
  const dateTimeString = getKoreanDateTime();
  const koText = `Playwright KO ${dateTimeString}`;
  const enText = `Playwright EN ${dateTimeString}`;

  // 1. 로그인
  await page.goto('/admissions/undergraduate/early-admission');
  await loginAsStaff(page);

  // 2. 편집 페이지로 이동
  await page.getByRole('link', { name: '편집' }).click();
  await page.waitForURL('**/admissions/undergraduate/early-admission/edit');
  ...
  await page.waitForURL('**/admissions/undergraduate/early-admission');
  ...
  await switchPageLanguage(
    page,
    'en',
    '/admissions/undergraduate/early-admission',
  );
